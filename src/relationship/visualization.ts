...
export class Visualization {
    ...
    private viewType: ViewType = 'cytoscape';
    ...
    /**
     * Cosmos.glを使用してグラフを表示
     * GPU加速WebGLレンダリングで大規模グラフに対応
     */
    public async showCosmosDiagram(symbols: SYMBOL.SymbolModel[], relationships: codeRelationships.Relationship[]) {
        const startTime = performance.now();
        this.viewType = 'cosmos';
        ...
        try {
            ...
            // ローディング画面を表示
            const html_loading = this.loadHtmlTemplate(path.join(this.extensionPath, 'templates', 'loading.html'));
            this.panel.webview.html = this.replacePlaceholders(html_loading, locale('window-title') + ' (Cosmos.gl)', { nodes: [], edges: [] });
            ...
            const html_cosmos = this.loadHtmlTemplate(path.join(this.extensionPath, 'templates', 'cosmos-view.html'));
            this.panel.webview.html = this.replaceCosmosPlaceholders(html_cosmos, locale('window-title') + ' (Cosmos.gl)', cosmosData);
    ...
    private replaceCosmosPlaceholders(template: string, title: string, cosmosData: cosmosAdapter.CosmosData, isStandalone: boolean = false): string {
        ...
        const replacements: { [key: string]: string } = {
            ...
            'GRAPH_SCRIPT_URI_PLACEHOLDER': isStandalone
                ...
                : `<script src="${this.panel!.webview.asWebviewUri(vscode.Uri.joinPath(this.extensionUri, 'dist', 'webview', 'cosmosView.js')).toString()}"></script>`,
